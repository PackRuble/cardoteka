...
///
/// Methods such as:
/// - [CardotekaCore.getOrDefault] (cannot be overridden to hide)
...
base mixin CRUD on CardotekaCore {
  ...
  /// Calls [Cardoteka.getOrDefault] method.
  ///
  ...
  V read<V extends Object>(Card<V> card) {
    ...
    if (instance case Cardoteka()) {
      return instance.getOrDefault<V>(card);
    } else {
  ...
  //
  // /// Calls [CardotekaCore.getOrDefault] method.
  // Future<V> readAsync<V extends Object>(Card<V> card) async =>
  //     super.getOrDefault<V>(card);
  //
